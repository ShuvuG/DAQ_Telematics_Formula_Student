...
unsigned long Sample_period; //Sampling period of the node: standard bus scheduling unsigned char Filter; //Use existing Kalman Filter: Yes(1) or No(0)
...
volatile unsigned long mask1; volatile unsigned long mask2; volatile unsigned long filter1; volatile unsigned long filter2; volatile unsigned long filter3; volatile unsigned long filter4; volatile unsigned long filter5; volatile unsigned long filter6;
};
struct Configuration {
unsigned char function_code; unsigned int Serial_baud; unsigned int Network_baud; unsigned int NODE_ID; unsigned int heartbeat_time; unsigned int error_repeat; unsigned int CS_pin; unsigned int interruptPin;
};
struct CAN_type{
...
Lookup_NODE Master = {0, 1, 9600, 15, 1000, 250, 2, 10, 1, 2, 10, 0, 0, 0}; //On how to synchronise all Lookup_NODE A = {0, 4, 9600, 15, 1000, 250, 2, 10, 1, 2, 10, 0, 0, 0};
Lookup_NODE B = {0, 120, 115200, 16, 1000, 250, 2, 10, 1, 2, 10, 0, 0, 0}; Masks_Filters Parameter2 = {1023, 1023, 0, 0, 0, 0, 0, 0}; //Masks and Filters CAN_type Shield = {2,10};
...
Serial.begin(Master.Serial_baud); if(!SD.begin(5))
...
SERIAL.println("CAN BUS Shield init fail"); SERIAL.println(" Init CAN BUS Shield again"); delay(100);
}
...
B.error_repeat, B.CS_PIN, B.interruptPin}; //CAN.sendMsgBuf (0x1, 0, 8, Slave1);
// CAN.sendMsgBuf (0x1, 0, 8, Slave2);
//synchronisation();// start interrupt }
//------------------------------------------------------------
// SEND CAN MESSAGES
//------------------------------------------------------------
...
CAN.sendMsgBuf(CAN_id, 0, data_length, CAN_message); extended, data length and data
...
Serial.print("\t"); }
Serial.println(); // */
//CAN_ID, type of message - standard or
...
/------------------------------------------------------------ // READ CAN MESSAGES //------------------------------------------------------------ char CAN_read()
{
unsigned char len;
...
Serial.println("Error control"); error_control(CAN_message); return 0;
} }
//checks whether data received from slaves //record CAN data in SD card or print on screen
//error control subroutine
//----------------------------------------------
// ERROR CONTROL //-------------------------------------------------- //prompted when 4 byte long error frame is received void error_control(unsigned char CAN_message[]) {
unsigned int timer1, timer2; // to idenitfy the producer time Serial.println("Starting error control protocol");
...
//Stores all raw CAN data in the CSV file, if not being overwritten on void CAN_receive( unsigned char CAN_message[])
{
unsigned long canId = CAN.getCanId(); timer = millis();
myFile.print(timer);

SERIAL.println("-----------------------------"); SERIAL.print("Get data from ID: 0x"); SERIAL.println(canId, HEX); myFile.print(id);
myFile.print(","); for(int i = 0; i<len; i++) {
SERIAL.print(buf[i], HEX); SERIAL.print("\t"); myFile.print(buf[i]); myFile.print(",");
} SERIAL.println(); myFile.println();
}
//Write node configuration parameters in a seperate CSV file in the raspberry pi //Currently only writes data for two nodes
/void fileWrite()
{
Node1 = SD.open("NODE1.csv", FILE_WRITE); Node1.println("NODE A configuration parameters:"); for (int i = 0; i< sizeof(A); i++)
{
Node1.println(buf[i]); }
Node1.close();
Node2 = SD.open("NODE2.csv", FILE_WRITE); Node2.println("NODE B configuration parameters:"); for (int i = 0; i<sizeof(B); i++)
{
Node2.println(buf[i]); }
Node2.close();
Master1 = SD.open("Master.csv", FILE_WRITE); Node2.println("Master configuration parameters:"); for (int i = 0; i<sizeof(Master); i++)
{
...
{
flagRecv = 1; }
//------------------------------------------------------------ // Start CAN //------------------------------------------------------------ void start_CAN(unsigned char global_timer)
{
//program these functions based on the the CAN start and CAN stop functionalities myFile = SD.open("can.csv", FILE_WRITE);
myFile.close();
if (timer > global_timer)
{
timer1 = timer;
Serial.println("Sending heartbeat message"); CAN.sendMsgBuf (0x700, 0, 4, heartbeat);
} }
//send heartbeat message
//need to use time before another reset can be sent //think about reset and
...
//heartbeat monitoring protocol needs to be worked on as a single entity Heartbeat_monitoring(NODE_ID, timer1, timer2);
timer2= timer1;
//functions compared between two consecutive time that the heartbeat message took
//If heartbeat message took more than twice to reach, node prints on the screen a message to check the node void Heartbeat_monitoring(unsigned int NODE_ID, unsigned int timer1,unsigned int timer2)
{
signed int n=0;
if (timer1> 2*timer2) {
